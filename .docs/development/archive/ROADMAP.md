...
This roadmap tracks work remaining after the shipped Windows multiplexer, explicit-Project,
and read-only control-plane phases. The original product contract is preserved at
`archive/AGENT_MUX_SPEC.md`; current contracts live in `../design/`.
...
Phases 0-7 are completion records. Their references to Spaces, app-owned space notes,
project scopes as session containers, or the pre-Project sidebar describe superseded
implementation stages, not current architecture.

## Current architecture reset (2026-07-16)

These decisions override conflicting language in completed phase records:

- Explicit Projects are the only session, layout, note, and file-resource containers.
  Every session has immutable `project_id` ownership and starts at the Project root.
- Groups organize Project rows in the sidebar only. They own no layouts or behavior.
- Spaces and session/agent-run/app notes are retired. Each Project owns one Markdown note;
  every session note action opens that note.
- Project files use a lazy tree, composable global/project ignores, bounded leased
  non-recursive watches, and revision-checked editors. Project notes autosave.
- Git status remains integrated; worktrees remain backend/API tooling rather than primary
  sidebar, tab, or session-creation concepts.
- swe-mux saves Claude/Codex auth snapshots, polls quota for active and inactive accounts,
  and selects one system-wide account per provider by replacing the normal auth file. It
  does not create concurrent provider homes or duplicate shared config/skills/history.

## Historical decisions for completed phases

These decisions explain the completed implementation sequence. The current architecture
reset and `../design/` are authoritative where language conflicts.
...
- Persistent Project/Group records and Project layouts, history/events SQLite index,
  external history reconciliation, transcript viewing/resume, Git polling/worktrees, and
  meta-hooks.
- Credential-free localhost/tailnet HTTP/WS surfaces, `mux` CLI basics, Project/session
  sidebar, up to four
...
- Claude/Codex account capture, system-wide selection, and non-blocking quota tracking for
  saved active and inactive accounts.
...
- [ ] Expand `mux` from a thin JSON client into a practical daemon controller: filtered
  session listing; Project-bound profile/custom-argv spawn; rename/pin/kill; Project/Group
  management; repository-group inspection; broadcast
...
- [ ] Expand Python coverage for configuration/migrations, adapter state races, session
  lifecycle, Host/Origin/WS boundaries, Projects/layout, history/resume, events/hooks,
  rules/annotations/budgets/OpenRouter fixtures, Project resources/account tracking,
  Git/worktrees, CLI behavior, process ownership, previews, and reaping.
- [ ] Add real-browser component/Playwright coverage for Project creation/folder selection,
  default/custom session creation, resources/autosave, non-split replacement, inline kill,
  Projects/panes, account switching, palette/input transparency, Settings, history,
  processes/previews, clipboard media, direct-tailnet use, responsive/touch, orientation
  changes, focus management, and accessibility.
...
- [ ] Persist opaque mappings from Telegram chat/message/thread/callback identifiers to mux
  session and Project ids. A reply to a notification targets its originating session; an
...
- [ ] Label every outbound prompt, approval, completion, and response with backend,
  session, and Project identity. Support Select active session, Open, Approve, Reject, Reply,
...
- [ ] Preserve one-user history semantics. Telegram may report the selected Claude/Codex
  account but never captures, selects, removes, or stores provider OAuth credentials and
  never creates a separate conversation archive.
