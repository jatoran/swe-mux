...
| Installer (`-setup.exe`) or portable archive | Press **Update** on the banner (or Settings → Maintenance → swe-mux version → Install). The dialog says what the update does before you confirm; the app is replaced in place and your sessions survive. Running the new release's installer over the top still works and is the path for a copy older than 0.2.4. |
