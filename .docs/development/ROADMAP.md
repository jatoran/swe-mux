...
- [ ] **Re-measure after v4 has run.** Compare the `abandoned` rate on wide-window records **before and after**, not the pooled fleet ratio: the pooled ratio moves for compositional reasons under any of these changes and would report success without any judgement improving. Pre-change wide-window baseline: **22.6% abandoned, 3 succeeded, n=62**.
...
- [ ] Verify on the isolated daemon that a live agent can read a sibling session's timeline digest and cursor a monitor poll, then commit and redeploy. **Note for whoever does it:** a read tool added after a session was spawned is not in that session's `--allowedTools`, so the check needs a freshly spawned agent.
