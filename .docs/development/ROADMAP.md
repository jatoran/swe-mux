...
Known before that test starts, from the build: a **codex** write is hashed as its `apply_patch` envelope rather than as whole-file content, so a codex contributor is expected to match by path and read `correlated`, not `exact`. A `correlated` codex contributor is the designed answer, not a failure of the run.
