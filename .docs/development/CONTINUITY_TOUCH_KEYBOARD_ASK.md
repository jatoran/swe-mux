...
swe-mux resolves SDK `^0.2.40` from npm as of 2026-08-29 - the tarball under `frontend/vendor/`
is gone, and the pinned identity is `package-lock.json`'s
`sha512-unWbWij2gpCAOdn4qY+zJjXWVGaf4fMeh6NZ2JxXB57Btlb3dp7rCc69VhNgaLDVjqkUnOF3uvEUIDwzVHoRTA==`,
which npm verifies on every install. 0.2.40 carries this keyboard behavior forward unchanged
from 0.2.35.
A version string is not an identity, and the registry is what settles it now. 0.2.35 shipped
twice under one version (`a45856c9…`, then the rebuild `af0ceb46…`), and 0.2.40 still exists in
two non-identical builds: the npm artifact and continuity's local
`target/sdk-release/sdk-v0.2.40/` output differ in `internal/continuity_wasm_bg.wasm` and
`integrity.json`, with every other difference being CRLF-vs-LF. The lockfile integrity is the
digest to verify against; a local build of the same version is not the same bytes.
