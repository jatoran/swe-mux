...
- Metered on the shared ledger under `builtin:attention-narration`, and enforced against the **global** automation caps: `automation_daily_budget` and the shared per-call `automation_max_output_tokens`. Its dedicated daily budget and output ceiling were retired with the scan timeline's (`budgets.md`); the switch and the read-only model row stay under Automation → Policy → Limits &amp; budgets → Attention.
