...
| How long after a paste may Enter be sent? | published `paste_submit_settle_seconds` / `paste_submit_settle_per_kib_seconds`, `paste_submit_settle_rules(name)` | The pace of every insert-and-submit, in the daemon queue and in the browser (`features/prompt-queue.md`) |
...
| Does the CLI export its own OTLP telemetry, and how? | `native_telemetry` (`transport`, `measured_version`, `runtime_call_id_prefix`, `http_requests_are_plumbing`, `exports_metrics`, `provides`), `code_mode_tool`, `hook_reports_runtime_layer` | The canonical ledger's exporter contract, invocation-layer classification, and code-mode transport (`telemetry_otlp.py`, `telemetry_schema.py`, `telemetry_ledger.py`); a harness with no entry is reported unsupported, never given another vendor's contract. `exports_metrics` is what wires a metrics exporter beside the logs one, and `provides` names the facts the export was *measured* to carry (`telemetry_otlp.CAPABILITIES`), so the quality view reports an absent fact as unsupported for that harness rather than as missing on every row |
...
- Every harness declares its terminal surface traits: `webgl_unsafe`, `owns_scroll_viewport`, `applies_width_envelope`, `min_desktop_columns`, `suppresses_late_color_response`, `touch_scroll_rows_per_report`, and `app_tail_reset`.
...
- `app_tail_reset` says what returns a TUI's *own* scroll viewport to its newest output other than the scroll the browser forwards, which is the only movement the browser can count (`features/ui.md` carries the estimate it corrects).
  It is `none`, `submit`, or `input`, and the three are ordered by how much has been established rather than by preference.
  Claude declares `submit`: a submitted prompt appends to the conversation and the CLI follows its own newest output, while nothing has established that an ordinary keystroke does the same, so the declaration stops where the evidence does.
  Every other harness declares `none`, which is also what a payload predating the field means, and `none` preserves the estimate exactly as it behaved before.
  Declaring too little costs one tap on a chip that outstayed its scroll; declaring too much withdraws that chip from a reader who is still scrolled up and leaves them no way back, so an unmeasured composer is `none`.
...
- Every harness declares how long it needs between a paste and the Enter that submits it: `paste_submit_settle_seconds` plus a `paste_submit_settle_per_kib_seconds` term for the payload.
  A CLI applies a paste on its own render loop and turns a large one into a placeholder chip; an Enter that arrives while that is still happening is not merely dropped - on Codex it lands in the composer as a newline, and the body sits there looking sent.
  This was one global constant until 2026-09-04, and the constant was Claude's, which is what made the Codex case unfixable without slowing Claude down: measured against the incident, four of seven 3.4-4.3 KB relay messages were never submitted at a 0.46-0.52 s settle, and the one retry (~1.1 s total) rescued one of them.
  Codex therefore carries 0.6 s + 0.25 s/KiB and every other row carries the historical 0.18 s + 0.08 s/KiB, which is also what an unmeasured harness keeps.
  The numbers are a first guess and are inferred from an incident rather than from a rig; the queue's escalating submit ladder is what actually guarantees delivery (`features/prompt-queue.md`), so a later measurement changes one row here and nothing downstream.
