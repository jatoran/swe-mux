...
| Does the CLI export its own OTLP telemetry, and how? | `native_telemetry` (`transport`, `measured_version`, `runtime_call_id_prefix`, `http_requests_are_plumbing`), `code_mode_tool`, `hook_reports_runtime_layer` | The canonical ledger's exporter contract, invocation-layer classification, and code-mode transport (`telemetry_otlp.py`, `telemetry_schema.py`, `telemetry_ledger.py`); a harness with no entry is reported unsupported, never given another vendor's contract |
...
- Every harness declares `publishes_cli_state`, `branch_strategy` (or `None`), `requires_direct_entrypoint`, `native_telemetry` (or `None`, measured against a real run before it is declared), `code_mode_tool` (or `None`), and `hook_reports_runtime_layer`.
