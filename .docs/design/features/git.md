...
- The "Reading this worktree…" placeholder sits **in the slot the change groups occupy**, so the arriving lists replace it in place.
  Above the acts it pushed them down the moment the row opened and let them spring back when the lists landed, which moved the Auto-merge button out from under a pointer already travelling to it.
  ...
  "Stuck on a human" is `approved || runs_without_approval`, never `approved` alone: a Project may let its own agents' gate edits run, and unfolding the strip over a gate that is about to run reports an emergency the operator already answered (`land-queue.md`).
...
- Each collapsed Map row gives the branch identity its own bounded line and wraps status metrics on a separate line, so divergence and state cannot overlap the title at narrow drawer widths.
  ...
  Branch identity is the row's strongest text, Git metrics stay quiet and unboxed, and the separate live-session action is a text affordance rather than another bordered badge.
- A worktree with an active land request adds one operational line to its collapsed row.
  ...
  A full-height leading rail uses the same state colour, giving the list a scan target that remains visible when the detail text truncates.
...
- The **land queue** is the one path that merges, and it is not this surface: it is a daemon-owned pipeline with its own fixed vocabulary, its own preconditions, and its own approval (`land-queue.md`).
  Its controls in this tab - the Auto-merge button on a Map row, and the auto-merge strip's cancel, verification-command editor, and approval - enqueue and cancel *requests*, write one Project config key, and approve the bytes that will run.
