...
authority to the surface.
`notify(envelope=...)` adds none either: it may only ask for *more* metadata about the sender
than the target Project's `message_envelope` requires, never less, so a repository that wants
the full trust statement gets it whatever a peer asks for, and the level actually used comes
back in the result rather than being applied silently. `notify(delivery="now")` asks for the item to be
...
**A refusal raised by the service behind a tool is that tool's answer, not a daemon fault.**
...
the generic handler above it can only say that something went wrong.
