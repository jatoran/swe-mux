...
- Every terminal has an in-flow **Action rail** at the bottom of its pane on desktop and mobile, below the terminal rather than over it.
  ...
  It carries the selection readout only, which is a state and belongs beside the keys that act on it.
  ...
  It is emptied rather than unmounted, so the live region is in the accessibility tree before the text it announces arrives.
