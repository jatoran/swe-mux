...
- **The Project menu's first row is `Run`, and it is the Run menu itself rather than a row about running.**
  It carries the `MenuGroup` caret because it expands, and it opens the way a `MenuGroup` does, decided by the same capability test (`useFlyoutCapable`): on a hover-capable pointer in the wide layout, hovering the row opens the ordinary `ProjectRunMenu` beside it after `MenuGroup`'s open delay, overlapping the row's right edge the way a `MenuGroup` flyout overlaps its header, with the Project menu still open beneath it; on touch or a narrow layout, tapping the row hands off to the Run menu and closes the Project menu, the same hand-off every `▶` trigger performs.
  The flyout (`RunMenuState.flyout`, `openRunFlyout` in `frontend/src/App.tsx`) has no scrim, because the Project menu underneath has to stay live, and the outside-press dismiss that closes context menus ignores presses inside the Run menu and its dialogs so a press there cannot close the Project menu the flyout belongs to.
  Leaving the row or the flyout schedules the flyout closed after `MenuGroup`'s close delay, and entering either cancels it, so a diagonal from the row into the flyout's far rows holds; that hover-close is the only way out that keeps the Project menu open, and every other way out - Escape, the flyout's `×`, launching anything from it - closes both, as it would for a `MenuGroup`.
  A sub-surface of the Run menu (trust prompt, input prompt, actions editor, worktree form) suspends the hover-close for as long as it is up: the pointer is on a form, and a pointer drifting off a form must not take the form with it.
  Opening the flyout folds an open Group flyout, and the flyout follows its Project menu: when that menu closes for any reason, or another Project's replaces it, the flyout goes with it.
  It is not a `MenuGroup`: the Run menu's dialogs and form cannot live inside a group whose only content is rows, and the accordion presentation on touch would fold a menu of that size into a menu of nine rows.
...
- **A ringed pad commits on release throughout, and that is geometry rather than taste.**
  ...
  The one-ring pad has no transit and keeps entry-firing, which is what makes the arrows fast.
  An explicit per-slot mode still wins, so the trade stays available to anyone who wants it.
...
- **Pad slots never reflow.**
  A slot whose Action this backend or session state does not offer stays exactly where it is and goes dead, rather than the others sliding over to fill the gap: wedges are positional, and a pad that rearranged itself per backend would be worse than one with a dead wedge.
  An empty wedge is inert for the same reason it is useful — it is a safe place to abort a drag into.
...
- The right-edge **utility drawer** is where the app's lookup and injection surfaces live, so they are one gesture on mobile or one visible click on desktop away instead of two menu levels deep.
  ...
  **Two tabs badge, and they mean different things** (`drawerTabBadge` in `frontend/src/drawerTabs.ts`, the one rule both the drawer's strips and the collapsed launcher draw from).
  ...
  Unlike the alert badge it stays inside the button box, smaller and tinted rather than solid: an overhang past the foot is clipped on both surfaces (the title-mode launcher button is `overflow:hidden`, the drawer strip is an `overflow-x:auto` scroller), where the head's overhang is absorbed by the rail's top padding, and a count of your own staged messages should read at a glance without competing with an alert.
