...
- **There is one writing surface and it is a queue row** (2026-08-31).
  ...
  **And the compose control belongs to the list rather than to the panel's chrome.**
  As a pinned footer it sat at the bottom of the tab whatever the list did, which drew a button a screen below the last item on a two-item queue and floated it under a lone paragraph on an empty one.
  It is the list's last row instead, in three placements: **centred** in the tab when nothing is queued, since there the button is the whole content and its explanation belongs beside it rather than in place of it; **directly under the last item** when the list does not overflow; and **stuck to the scrollport's edge** once it does, because a row that travelled with the content would put "add" a scroll away on a long queue.
  Sticky is what makes those one rule rather than a measured branch, and it needs two things a plain `bottom:0` does not give: an **opaque** background, since rows passing under a transparent control read as a rendering fault, and a **negative offset of the list's own bottom padding**, because a sticky offset is measured against the scrollport minus that padding and `0` therefore parks the row a gutter short of the edge with items scrolling visibly through the gap.
  The gutter is one CSS variable for that reason - two rules holding the same number by hand is where it comes back.
