...
| Automation workspace (agent authority rows) | every authority field, plus `agent_authority_default` and `agent_authority_ceiling` | install + Project | owner — the one surface that may lower one |
...
That is not a further rule but the first one applied honestly: a grant that cleared a block and left the queue empty would be the enabled-and-does-nothing state this design exists to prevent.
