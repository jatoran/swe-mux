...
| Prompt queue | `queueApi.ts`, `QueuePane.tsx` | typed client over `/api/queue/*` with the refusal→outcome mapping (`mapQueueSendError`), head-of-line/pending selectors, and the per-target Queue tab (list, arm/edit/reorder/cancel/skip, send-now + confirm, stranded retarget, composer); the daemon owns every safety decision, `App.tsx` owns tab placement, the pane chip, and `mux:queue-changed` re-dispatch |
