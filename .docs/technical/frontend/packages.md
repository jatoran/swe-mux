...
| Utility drawer | `drawerLayout.ts`, `UtilityDrawer.tsx`, `drawerTabs.ts`, `drawerNotes.ts`, `sidebarResize.ts`, and feature-named tab bodies | `drawerLayout.ts` owns the JSX-free device-local recursive tree and per-Project presentation algebra, `App.tsx` owns migration and atomic persistence, `UtilityDrawer.tsx` owns desktop recursion plus the flat mobile projection and singleton body dispatch while delegating rail overflow mechanics to `OverflowRail`, `drawerTabs.ts` remains the registry, Notes alone keeps one hidden inactive host, and Processes consumes App's fleet snapshot without another poll. |
