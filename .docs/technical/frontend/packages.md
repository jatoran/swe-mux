...
| Agent Context | `AgentContextTab.tsx`, `agentContext.ts` | Project/global read-only instruction inventory and viewer; desktop right-click reveal menu over opaque source IDs; one collapsed counted Memories disclosure; focus-trapped modal for Project-root diff/confirm sync in either direction plus revision-guarded restore points. It never accepts paths, edits bodies, writes global instructions/provider memory, or auto-syncs |
