...
| Mobile soft keyboard | `mobileKeyboard.ts`, `mobileTerminalIme.ts` | `mobileKeyboard.ts` decides which focused element holds the on-screen keyboard up and performs the one blur that lowers it. Its predicate and shadow-root walk (`deepActiveElement`, because `document.activeElement` retargets to a shadow host and the Continuity editor's `<textarea>` lives behind one) are pure and duck-typed. `mobileTerminalIme.ts` converts Android composition replacements to incremental PTY deltas and owns the backend-aware Enter payload: agent newline (`ESC+CR`) versus shell submit (`CR`). `TerminalPane.tsx` applies it consistently to `keydown`, `beforeinput`, and value-delta fallback, and owns the fixed mobile agent Send end-cap plus sticky read/select mode. |
  | Terminal viewport | `TerminalPane.tsx`, `RailScroller.tsx`, `railOverflow.ts`, `terminalCaretPlacement.ts`, `railKeyRepeat.ts`, `terminalAttachments.ts`, `terminalProtocol.ts`, `terminalViewport.ts`, `terminalRenderer.ts`, `terminalRenderDiagnostics.ts` | xterm/WS lifecycle, provider-aware tap/click caret placement (native negotiated mouse forwarding for Claude touch; guarded redraw-feedback Left/Right steering for Codex), file picker/drop/paste attachment uploads and unicast draft references, endpoint-aware command-rail overflow controls and pure paging/focus math, pre-replay attach sizing, renderer policy/fallback (DOM-only for mobile and Codex), replay, device-response classification/Codex late-color suppression, input, non-focusing pointer-hold repetition for the command-rail arrows, responsive fitting, jump-to-latest tail convergence and which backends are also asked to move their own viewport, dev-only render diagnostics |
...
| Utility drawer | `drawerLayout.ts`, `UtilityDrawer.tsx`, `drawerTabs.ts`, `drawerNotes.ts`, `sidebarResize.ts`, and feature-named tab bodies | `drawerLayout.ts` owns the JSX-free device-local recursive tree and per-Project presentation algebra, `App.tsx` owns migration and atomic persistence, `UtilityDrawer.tsx` owns desktop recursion plus the flat mobile projection and singleton body dispatch, `drawerTabs.ts` remains the registry, Notes alone keeps one hidden inactive host, and Processes consumes App's fleet snapshot without another poll. |
