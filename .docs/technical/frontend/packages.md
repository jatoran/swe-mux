...
| Horizontal overflow rails | `RailScroller.tsx`, `railOverflow.ts`, `wheelScroll.ts` | shared endpoint detection, overlay fade chevrons, tab-boundary paging, wheel translation, and selected/focused item reveal for workspace tabs, utility tabs, and the terminal command rail; callers retain tablist semantics, drag targets, and native touch/trackpad scrolling |
