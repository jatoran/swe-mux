...
`pendingUtterance` is the third pure export: it composes the panel's client-local pending row from the brainstorm buffer, the pen's held fragment, and the speculative decode's provisional reading of the breath in progress, in the order they were spoken.
