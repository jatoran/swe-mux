...
It opens itself only while a land is stuck on a human - the install stop is off, written gate bytes are unapproved, or a worktree's own copy of the gate refused a land - which is `landingSummary`'s `opensByDefault`, deliberately narrower than "this tab cannot land anything": a repository with no verification command cannot land either, and stays folded, because that is the resting state of every repository that never opted in rather than something a person is stuck on.
