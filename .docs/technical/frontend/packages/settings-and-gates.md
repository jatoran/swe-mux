...
`OnboardingFlow.tsx` owns the resumable sequence, with `HarnessSetup.tsx` providing its experience and harness pages.
`onboarding.ts` reads daemon-owned progress and retries failed startup reads.
`GettingStarted.tsx` renders the persistent sidebar section.
Provider connection controls, key operations, model routing, preset previews, and budgets are shared with Settings.
The UI tour follows setup and persists its current step on the daemon.
...
`GuidedTutorial.tsx`, `tutorial.ts` - action gates, coach-mark geometry, product-event matching, and host-managed progress.

The pure decisions in `tutorial.ts` are consumed by `App.tsx`, which is what makes them testable without mounting the shell (`test/tutorial.test.ts`).
...
`firstRunSurface(state)` returns the one first-run surface that may render - `'harness' | 'tutorial' | 'none'` - using the durable onboarding status and suppressing both while another setup guide is open.
...
## Setup and learning integration

`OnboardingFlow.tsx` owns the four core pages, optional permission/model pages, and focused extras.
`HarnessSetup.tsx` renders experience and agent cards, while `setupHarnesses.ts` owns detection and retry.
`setupDiscovery.ts` keeps per-harness background requests alive across pages and publishes independently completed folder lists to `SetupProjects.tsx`.
`SetupKeymap.tsx` opens the existing Settings editor for customization; its applied-preset marker prevents the return path from overwriting custom bindings.
`SetupPermissions.tsx` renders daemon-owned autonomy assignments and ordinary validated refinements.
`ProviderConnectionFields` and `providerKeyOperation` are shared with `Settings.tsx`.
`SetupModelSummary.tsx` renders two read-only model rows with per-role Change controls; the complete feature routing editor remains `ModelRoutingSummary.tsx` in Settings.
`setupActivation.ts` grants only pending model features after verification.
`VoiceSetup.tsx` reuses acquisition panels and the ordinary capture/transcription path, and completion requires functional confirmation.
`useSetupDraft` in `onboarding.ts` merges optimistic non-secret edits with serialized progress writes.
`ExperiencePreview.tsx` renders the daemon-owned preset assignments in both setup and Settings.
`GettingStarted.tsx` distinguishes task completion, dismissal, section collapse, and hiding.
`GuidedTutorial.tsx` accepts a saved step and reports progress through its host, omitting account setup after onboarding.
`ConnectPhone.tsx` renders the same remote connection components as Settings with guided actions and user-confirmed completion.
