...
`OnboardingFlow.tsx` sequences `HarnessSetup.tsx`, `ProviderSetup.tsx`, `SetupAccounts.tsx`, `SetupProjects.tsx`, and `DesktopSetup.tsx`.
`ProviderConnectionFields` and `providerKeyOperation` are shared with `Settings.tsx`; model-role controls remain generated by `ModelRoutingSummary.tsx`.
