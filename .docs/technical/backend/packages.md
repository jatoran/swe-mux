...
| `terminal_arbitration.py` | pure multi-device rules for one shared PTY: input-ownership claims (gesture beats passive; passive cannot displace an actively typed-into owner, come from a hidden window, or cross to the device class in use — and does cross from an idle class when the claimant's is in use), epoch/release bookkeeping, and the arbitrated geometry (owner's viewport, else the smallest visible one) | sockets, PTYs, telemetry, and *which* device is in use — `device_presence.py` answers that, `session.py` holds the state, `server.py` applies the decisions |
