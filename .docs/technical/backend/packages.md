...
| `voice.py` | completed-reply TTS segments with one-shot summary/verbatim overrides, bounded application-text speech, bounded Whisper STT with GPU/CPU fallback, temporary audio lifecycle, voice-submit idempotency, and one-use approval challenges bound to the current screen fingerprint | browser microphone permission, PTY ownership, approval-state classification |
