...
| `server.py` event/PTY transport | cold event watermarks, 64-record reconnect recovery, snapshot fallback for wider gaps, browser omission of audit-only hook payloads, and zero-delay batching of already-queued PTY output | durable event storage, browser state ownership, transport-level compression |
