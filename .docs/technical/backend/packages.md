...
| `assistant.py` | the Mux assistant (Phase 10.6): daemon-owned dialogs/messages/actions in SQLite, the bounded tool-calling turn loop, name→entity resolution with candidate answers, the per-class trust policy (read/navigation/reversible/consequential with a non-configurable consequential confirm floor), UI-command dispatch to the originating device with bounded acknowledgement, `builtin:assistant` ledger spend and daily-budget refusal | executing anything outside injected daemon operations, emitting identifiers from the model, PTY writes, the reflex voice path |
