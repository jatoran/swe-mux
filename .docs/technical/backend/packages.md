...
| `mcp.py` | agent-facing MCP protocol + tools: four read tools (session list/status, bounded transcript read, history search) and two thin write callers (`notify`, `request_spawn`), token-derived caller identity, Project scoping, UI-equivalent session display names and exact unique display-name resolution, output redaction | relay policy and bounds (those live in `agent_messaging.py`), title generation (read from `automation_store.py`), delivery, PTY writes, spawn, aiohttp handlers (`server.py`) |
