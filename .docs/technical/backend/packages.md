...
- Plugin host - `plugin_manifest.py`, `plugin_store.py`, `plugins.py`, and `routes/plugins.py` own inert manifests, direct-child development discovery, read-only release-channel checks, durable update review, lifecycle, scoped callbacks, pane restart, command execution, EventBus hooks, validated catalog consumption, and management routes.
