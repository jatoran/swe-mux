...
| `composer_input.py` | pure estimate of unsent composer text from the bytes written to a PTY: what each write does to it (typed, erased, submitted, discarded), bracketed-paste state carried between writes, and the empty ↔ non-empty crossing that is the only reportable event. Which keys count as a discard is a *parameter* (`clear_keys`) rather than a constant, supplied from `HarnessDescriptor.composer_clear_keys`: Ctrl+U is a line kill in Claude Code, so the old fixed set read a one-line kill on a four-line draft as "now empty" — the false safe this module exists to avoid. The declared sequence is matched against the raw frame, since Claude's is itself an escape sequence and the escape stripper would have eaten it. `server.py` calls it wherever `input_revision` is advanced; `session.py` clears it when a turn opens or the session ends | reading the composer (nothing can), authorizing anything — `delivery_readiness.py` keeps its own coarse boundary and never consults this |
...
| `projects.py` | Project/Group validation and lifecycle, including tombstoned removal, same-root identity restoration, monotone shared explicit-use recency, and leaf-validated create-mode folder minting | Git-derived identity, file content |
| `leaf_names.py` | the shared Windows-safe filesystem-leaf validator (invalid characters, reserved device stems, caller-supplied control-directory refusals) and the deterministic name→folder normalization mirrored by the frontend's `suggestFolderName` | path containment (`path_identity.py`), any filesystem access |
| `git_review.py` | Project-scoped comparison-ref inference (`resolve_comparison_ref` is the shared core; `infer_comparison` adds the drawer's selector candidates and `git_monitor` deliberately imports the core so the sidebar and the drawer cannot pick different bases), exact worktree and relative-path validation, single-flight overview reads, per-worktree branch-tip committer dates (`head_commit_dates`, one batched `git show` against the shared object database keyed back by the oid asked for — deliberately not the checkout directory's `st_mtime`, which Windows freezes while a live session holds a file open there, so a directory clock reports the busiest worktree as the most dormant and any activity ordering built on it is inverted; reading from the object database also means a locked or prunable tree still dates), file-count and aggregate-byte-bounded worktree/commit summaries, numstat and porcelain parsing, commit graph reads, capped commit-message reads, patch snapshots, stale hashes, and typed review failures | network fetches, Git mutations, live-session polling, browser presentation |
| `git_monitor.py` | bounded read-only (`--no-optional-locks`) polling of every session's checkout for full HEAD, branch, dirty count, upstream divergence, worktree identity, root, HEAD-scoped diffstat, and merge-base comparison counts; same-checkout previous-HEAD event evidence; bounded commit-range, changed-file, combined-merge-diff, excluded-range and blob-digest readers for attribution; deduplication by `(cwd, comparison override)`; memoization of both diffstats and of ref inference | comparison-ref inference itself (`git_review`), Project registry lookups (injected as a callable), Git mutations |
...
| `git_provenance.py` | content-free event correlation around explicit successful commit-creating tool calls (`commit`, `merge`, `cherry-pick`, `revert`, `rebase`, `am`) and monitor-observed HEAD transitions; reference-move classification from first-parent range plus two ancestry answers; authored-versus-arrived separation with a time-ordered ledger arrival oracle; committer isolation by commit range, message subject and command window; integrator classification of a merge commit's creator and combined-diff scoping of what a merge itself resolved; branch authorship of a merge derived from the ledger's own rows for the commits its first-parent side had; bystander suppression once a committer or integrator is known; contributor matching of Tier 0 write facts to a commit's changed files with content confirmation; immutable commit-metadata capture; ranked durable upsert; per-commit rollup; bounded pending state, attribution and move memos, and health diagnostics | shell interpretation, repository mutation, Git author identity, injected commit identity, browser presentation, claiming authorship of a commit that merely arrived, crediting a merge's creator with the branch content it carries, putting a branch author's paths on a merge they did not write |
| `git_provenance_backfill.py` | explicit read-only planning and idempotent batch import from provider-native tool calls/results; fixed Git object inspection; ancestry re-attribution of rows the retired shared-checkout rule downgraded; bounded historical contributor derivation through `git_provenance`'s own matchers; retroactive integrator reclassification of recorded merge commits and branch-author derivation for them, planned before the withdrawal pass so a promotion and a retraction cannot name one row; reclassification of monitor occupancy through `git_provenance`'s own move classifier, withdrawing arrivals and answered bystanders and reconsidering only its own prior verdicts; single-Project or all-Project sweeps; aggregate-only rotating audit log | transcript mutation, transcript-command execution, automatic startup migration, author identity claims, withdrawing a row retracted by anything but itself |
| `project_files.py` | safe Project config (including `read_project_config_values`, the named accessor for the *values* rather than the envelope - handing a consumer the envelope reads as "this Project configured nothing" with no symptom, which is how `[worktree] verify_command` came to be inert for the land gate), flat Project-note collection, legacy-note migration, recoverable note deletion into `notes/trash/` plus the last-note `ProjectNoteProtected` refusal, missing-root read safety, lazy global Scratchpad storage, tree, exclusive leaf-only file/folder creation, bounded recursive name/content search, revision-checked text reads/writes, and allowlisted image inspection/content with byte, dimension, pixel, and frame limits | layout placement, browser drafts, generic browser MIME rendering, generic browser-file overwrite/move/delete operations |
| `worktree_setup.py` | Project-config/convention setup resolution, supervised pre-spawn subprocess execution, timeout/tree reap, bounded output capture, public result and terminal prelude | Git worktree creation, harness trust, session spawning; command resolution and bounded execution themselves (`worktree_exec.py`) |
| `worktree_exec.py` | the shared seam under both repository-declared worktree commands: `[worktree]`-override-or-executable-script resolution, shebang resolution on Windows, head-and-tail bounded output capture with an optional `on_chunk` observer whose exceptions are contained (a throw there would abandon a half-read pipe and block the process it is draining), and one timeout/tree-reap subprocess runner that returns the exit status exactly as given | authority of any kind - bootstrap has none to check and verification's lives in `worktree_verify.py`; interpreting what the observed bytes mean (`verify_progress.py`) |
| `worktree_verify.py` | the land gate's authority: `[worktree].verify_command`/`.worktree-verify` resolution, the machine-local exact-content approval store (digest over source-kind plus bytes, retained snapshot for the diff, un-approved by any edit), and the typed run result whose exit code is never re-derived and which now also carries the steps the run announced and its line count | execution mechanics (`worktree_exec.py`), when a gate runs (`land_queue.py`), how progress is read (`verify_progress.py`), HTTP |
| `verify_progress.py` | what a *running* verification gate says about itself, read off its own output stream: `=== name ===` step boundaries (exactly three equals signs each side, and a captured name carrying one is rejected, so pytest's own section rules are not read as steps), per-step and per-run elapsed time, a line count for a gate that announces nothing, and a snapshot whose `expected_step_count` is `None` unless a byte-identical run has already passed - withdrawn again the moment a run overruns that plan. Bounded (tracked steps, name length, line buffer) and exception-free by construction, because it runs inside the pipe reader | any estimate, any percentage, deciding when to record a plan or where to store one (`land_queue.py`, `land_store.py`), the process itself |
...
| `spawn_contract.py` | spawn field validation: bounded env, cwd containment, Claude marker scrubbing, model resolution against the harness registry (`resolve_spawn_model`, `apply_spawn_model`) | project ownership (the caller supplies the root); which backend a request resolves to |
...
| `transcript_view.py` | bounded harness conversation parsing; normalized native tool names/inputs with results excluded; branch linearization (`_claude_live_uuids`/`_mark_abandoned_records`: the live branch is the newest record's ancestry plus the tool results, sidechains, and attachments hanging off it - `design/features/transcript-branches.md`), dropped by the indexing projection and marked by the reader; the human-readable `conversation_view` reduction (CLI machinery classified out, agent turns merged per tool-free segment and never across a branch boundary, call details carried at each boundary and after the newest message, byte/message capped, own LRU); `conversation_cut_points`, the byte span each displayed message occupies and how many tool calls a cut there would leave unanswered, per dialect and `None` where no rule is measured, counting only calls the live branch made, with `resolve_cut_offset` turning a chosen message and side into that byte offset (shared by the interactive branch picker and a scheduled fork, so the two cannot disagree about which cuts are legal); `final_exchange`/`final_reply_text`, the one definition of "the agent's latest reply" shared by the reader tab, the rail's copy, and read-aloud | process state, transcript writes, tool results, telemetry, persistence, redaction (the reader is the machine's owner) |
...
| `automation.py`, `automation_store.py` | bounded rule evaluation and observer lifecycle, including provisional/settled title state, retries, budgets, append-only annotations, and scan-record queries whose time, trigger and semantic predicates (`json_extract`) run in SQL so a bounded page means rows *returned* rather than rows scanned | PTY writes, provider transcript mutation, browser presentation |
...
| `voice.py` | completed-reply, manual, and application-text TTS streams with a coherent sentence-first clip and tracked segment-tail tasks; open application-speech streams (`SpeechStream`, one worker per stream so clip indices stay monotonic, tighter opening clip because that clip is time-to-first-sound); one-shot summary/verbatim overrides; bounded Whisper STT with GPU/CPU fallback; temporary audio lifecycle; compatibility voice-submit idempotency; one-use approval challenges bound to the current screen fingerprint | browser microphone permission, mounted-composer state, PTY ownership, approval-state classification |
| `kokoro_tts.py` | the direct-onnxruntime Kokoro-82M engine: espeak-free misaki G2P (`fallback=None`, loud refusal if an espeak wrapper is importable), the out-of-vocabulary repair ladder (lexicon → compound splitter → spell-out, replacements re-verified), phoneme-token chunking, WAV synthesis with GPU providers when present | model acquisition (that is `voice_models.py`), any wrapper library (they carry GPL espeak payloads), network access |
| `voice_models.py` | the pinned, per-file SHA-256-verified Kokoro model download under `<data_dir>/voice-models/kokoro`: explicit not_downloaded/downloading/ready/error state whose error can never load, progress callbacks, restart-interruption detection | bundling models, loading them (that is `kokoro_tts.py`), any unpinned revision |
| `assistant.py` | the Mux assistant (Phase 10.6): daemon-owned dialogs/messages/actions in SQLite, the bounded tool-calling turn loop with sentence-granular streaming (`_SentenceStreamer`), a per-round budget line the model plans against plus an announced (never silent) exhaustion, a one-deep merge queue for utterances that arrive mid-turn, and speech suppression only when a lone confirmation card is the turn's whole outcome, the per-dialog action ledger and identical-proposal guard that keep a confirmed write from being proposed (and written) twice, the announcement-restarted cancel window, name→entity resolution with candidate answers, the per-class trust policy (read/navigation/reversible/consequential with a non-configurable consequential confirm floor), name-only project creation preflight (folder leaf from `leaf_names.suggest_folder_name`, parent from `new_project_parent` only), UI-command dispatch to the originating device with bounded acknowledgement, the Project Action pair (list with per-file approval state; run one approved action on the consequential floor, refusing an unapproved one by naming its file) and the bounded post-run outcome watch that reports one terse notification - success or an issue flag - and never reads a step's output back, `builtin:assistant` ledger spend and daily-budget refusal | executing anything outside injected daemon operations, approving a Project Action or running an unapproved one, quoting task output in an outcome report, emitting identifiers from the model, PTY writes, the reflex voice path |
...
| `automation_registry.py` | control-plane enablement DAG: substrate/consumer deps, cycle-checked resolution, whether an entry `spends`, whether it `needs_llm`, and the model-free starting set a new Project is offered. `resolve(..., llm_ready=False)` moves the `needs_llm` entries out of `enabled` into `unverified` - its own field, not a `blocked` entry, because `blocked` values are ids a grant can switch on and no automation's enabling fixes an unproven endpoint | storage, execution, knowing whether a provider is proven (that is the caller's answer to pass in) |
| `budget.py` | the one spending-cap shape and the only two comparisons that enforce it: `Budget{tokens?, usd?, mode}`, which axes a mode enforces, `spent_out` (inclusive) and `would_exceed` (strict preflight), the `BudgetVerdict` sentence a surface renders verbatim, and `cost_blind` - the statement that a dollar axis cannot bind over calls whose provider reported no cost | reading config (`config.py` owns the fields, their bounds, and the migration), reading the ledger (callers pass a `spend()` row in), deciding what an exhausted budget *does* to a feature |
| `llm_endpoint.py` | which language-model endpoint the install talks to and whether it is proven: the `LlmEndpoint` descriptor (origin, secret name, single-model override, cache policy, and what OpenRouter-specific behaviour does or does not apply), base-URL and model validation, the fingerprint over the whole `{base_url, model, api_key}` triple that makes an edit un-verify the endpoint by construction, and the `readiness` verdict with the sentence a surface renders verbatim | HTTP (that is `openrouter.py`), storage (that is `automation_store.py`), reading config files, deciding which *features* a verdict gates |
| `grants.py` | what a gate may switch on and what it must never touch: the two closed allowlists (validated against `Config` and `PROJECT_CONFIG_FIELDS` at import), the additive-only rule, and the validated plan `server.apply_grants` applies. Turning something **off** is deliberately absent — that is the owning editor's, which is what lets many surfaces grant one switch | storage, execution, network |
...
| `scan_timeline.py` | three-gated, run-scoped Tier 1 scanner: event/heartbeat scheduling, forward (oldest-first) transcript windows with catch-up chaining, same-run continuity carrying the run-level verdict forward (`CONTINUITY_FIELDS`, prompt v4), user Project context and Tier 0 facts, cancellable and durably tracked full-session scans, strict DeepSeek V4 Flash extraction with repair-and-retry, rollover boundaries, its own daily/run/hourly budget enforcement, exact-interval source rehydration, dead-end annotation candidates, the stored-trigger vocabulary (`STORED_TRIGGERS`, wider than the event-bus set), and `liveness()` - the one owner of the enablement/liveness block the drawer snapshot and the `scan_timeline` MCP read both serve, including for an ended session | PTY writes, attention ranking, cross-run continuity, guessed records when the provider fails, the per-rule episodic-observer caps, record projection or digest shaping (`scan_consumers.py`) |
| `scan_consumers.py` | pure model-free derivations over the scan spine: phase segments, the bounded `catch_me_up` digest (most-recent segments, length-capped lines, and a count of what each bound dropped), the live-blocker streak, the distilled-record search, the Phase 7.11 compact `project_record` projection, and `repaired_fields`, which classifies the repair strings `_validate_semantics` writes so a cosmetic `behavior` dedup is distinguishable from an enum fallback | any read of its own (every function takes records in), model calls, storage, transport |
...
| `mcp.py` | agent-facing MCP protocol + closed tool set: own-Project-by-default session/run briefs, compact filtered history hits and stale-safe hit-neighborhood reads, bidirectional run-bound transcript pages, Agent Context sources, Project notes, sender message status, caller spawn-request status, the four Phase 7.5 cross-session memory reads (`provenance`, `verified_status`, `prior_resolutions`, `dead_ends` - DAG-gated per `MEMORY_TOOL_AUTOMATION`, run-attributed, `unsupported`/`disabled` over a fake empty), the Phase 7.11 scan-timeline reads (`scan_timeline`, session-scoped and gated on the *target* session's Project opting into `scan_reads`, digest/records/full details over the `scan_consumers` projection with an exclusive `since_t1` cursor and the liveness block on every result; `scan_search` over distilled records, gated on `semantic_history_search`), and the thin write callers (`notify`, `request_spawn`, `run_action`, the Phase 7.6 `interrupt`/`end_session`, and the Phase 14 `request_land`, whose worktree is read from the caller's own live cwd rather than accepted as an argument); token-derived identity, exact display-name resolution, cursors, output budgets, redaction, and content-free per-tool result diagnostics | history indexing/ranking (`history.py`), relay policy and queue/request storage (those live in `agent_messaging.py` and existing services), session-control authority and bounds (`session_control.py`), land authority and bounds (`land_queue.py`), title generation (read from `automation_store.py`), delivery, PTY writes, spawn, aiohttp handlers (`server.py`) |
...
| `land_queue.py` | Phase 14 `LandQueueService`: the install switch, the per-Project `land_queue` opt-in and `off`/`draft`/`granted` grant, the per-origin hourly budget, the supervised sweep, and the fixed four-command git vocabulary of reconcile / verify / fast-forward / abort, plus the bounded redacted handback template and the refusal-versus-hold decision. It owns the *lifetime* of a gate's progress reading - created per attempt, seeded from the recorded plan for that digest, attached to a `verifying` row by `status()` and dropped the instant the process ends, and recorded as a new plan only when the run passed | the precondition reads (`land_preconditions.py`), durability and serialization (`land_store.py`), the gate's authority (`worktree_verify.py`), how progress is parsed (`verify_progress.py`), delivery (injected `PromptQueueService.enqueue`), HTTP |
| `land_preconditions.py` | one consistent reading of both checkouts and the disposition it implies - `ready`, a transient `hold` (busy tree, working session, unreadable repository), or a permanent `refuse` (trunk that is a linked worktree, wrong branch, unregistered checkout); fails closed, and identifies the main tree by `--absolute-git-dir` versus `--git-common-dir` rather than by name | which sessions are busy (injected by `server.py`), mutation of any kind |
| `land_store.py` | `land_requests`/`land_events`/`land_verify_plans` on a dedicated worker thread: conditional state transitions, the per-step audit trail, restart recovery of orphaned steps, the two partial unique indexes that make one-request-per-branch and one-land-per-trunk properties of the schema rather than of the worker, and the upserted per-digest record of what a *passing* gate's steps were (a malformed one reads back as no plan, never as a wrong total) | policy, git, HTTP; the live reading of a running gate, which is deliberately never persisted |
...
| `schedules.py` | pure trigger arithmetic and validation for scheduled runs: the five-field cron parser (Vixie day-field union), interval and one-off triggers, the wall-clock abstraction with its host-local and named-IANA implementations, the two decided DST edges (spring-forward gap fires once, fall-back repeat fires once), the `spawn`/`resume` action and its three target kinds, bounds on every field (including the shorter `once` horizon a resume gets, because the harnesses prune their own transcripts), and the typed `ScheduleError` with its machine code and field map | storage, spawning, resuming, permission, any I/O |
...
| `scheduler.py` | the sweep and every fire-time guard: advance-the-window-first, claim-then-act, the install switch and the per-Project `scheduled_runs` opt-in checked at fire time rather than write time, the missed-window policy (`catch_up` replays once, otherwise `missed`), overlap, the per-schedule daily cap and install-wide concurrency ceiling, the resume path's target resolution and context ceiling, the transient-versus-permanent classification of a refused resume, message staging, and the outcome/notification record | the spawn itself (injected `_spawn_from_body`), the resume itself (`session_resume.resume_run`), delivery (injected `PromptQueueService.enqueue`), trigger arithmetic (`schedules.py`), HTTP |
| `session_resume.py` | the single resume authority, shared by the History route and the scheduled-resume path: the structural refusals (`not_agent`, `native_id_missing`, `cwd_missing`, `transcript_unavailable`, `target_project_missing`, `adapter_missing`), the two claim checks (a mux pane on the conversation, and `conversation_holder` for processes mux does not own), whether the resumed pane inherits the run row (the adapter's answer, never this module's), proof that the pane survived its settle window, `fork_run` for a fork-then-resume, and `resolve_latest_run`, which follows rollovers and `resume` edges and deliberately not `branch`/`review`/`handoff` ones | layout attachment and lineage (the callers', because only they differ on it), HTTP, trigger arithmetic, the fork writer's byte work (`transcript_fork.py`) |
| `preview_capture.py` | optional headless preview screenshot (Playwright), typed-unavailable | proxy transport, PTY writes |
| `preview_store.py` | the approved-preview mirror at `<data_dir>/previews.json`: load-at-startup, whole-file rewrite on change, and dropping stored rows that cannot route. Only *approved* registrations, because detected ones are rediscovered from the live listener set under the same derived id and mirroring them could only go stale | being authoritative at runtime (`PreviewRegistry.items` is), minting ids (`processes.preview_id` does), and ever failing loudly enough to stop the daemon starting |
