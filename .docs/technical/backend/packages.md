...
| `worktree_exec.py` | the shared seam under both repository-declared worktree commands: `[worktree]`-override-or-executable-script resolution, shebang resolution on Windows, head-and-tail bounded output capture, and one timeout/tree-reap subprocess runner that returns the exit status exactly as given | authority of any kind - bootstrap has none to check and verification's lives in `worktree_verify.py` |
| `worktree_verify.py` | the land gate's authority: `[worktree].verify_command`/`.worktree-verify` resolution, the machine-local exact-content approval store (digest over source-kind plus bytes, retained snapshot for the diff, un-approved by any edit), and the typed run result whose exit code is never re-derived | execution mechanics (`worktree_exec.py`), when a gate runs (`land_queue.py`), HTTP |
...
| `land_store.py` | `land_requests`/`land_events` on a dedicated worker thread: conditional state transitions, the per-step audit trail, restart recovery of orphaned steps, and the two partial unique indexes that make one-request-per-branch and one-land-per-trunk properties of the schema rather than of the worker | policy, git, HTTP |
