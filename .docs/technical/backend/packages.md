...
| `agent_context.py` | read-only Project instruction/provider-memory inventory and opaque source reads; normalized compare; preview/revision-guarded whole-file `CLAUDE.md` ↔ `AGENTS.md` sync; atomic replace and data-dir restore points | arbitrary browser-supplied paths, editing or writing learned memory, automatic sync, private Codex store formats, MCP exposure |
