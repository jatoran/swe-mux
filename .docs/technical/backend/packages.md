...
| `git_provenance_backfill.py` | explicit read-only planning and idempotent batch import from provider-native tool calls/results; fixed Git object inspection; evidence confidence and cross-session ambiguity classification; aggregate-only rotating audit log | transcript mutation, transcript-command execution, automatic startup migration, author identity claims |
...
| `history.py` | shared schema, Project/layout persistence, run history, bounded post-startup message-search repair with literal fallback, and durable session/run-to-commit provenance with evidence promotion and lifecycle cleanup | live PTY lifecycle, provenance evidence classification |
...
| `agent_messaging.py` | relay policy for agent-authored messages (Project scope, size, per-origin budget, target backlog, propagation depth, per-thread turn budget, ring detection, kill switch, expiry), sender-only message/request status, inert `spawn_request` drafts, and the Fleet Queue projection over messages plus targetless spawn approvals | delivery, spawning (approval is a `server.py` human act), MCP protocol |
