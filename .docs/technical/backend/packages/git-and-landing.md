...
And `_mark_absorbed`, which answers - per reading, never written back - whether the trunk has absorbed a standing refusal (`_absorption`: it contains the requested tip, or the branch's current tip, or both the requested commit and the branch are gone), so a branch landed outside the queue stops leaving a live-looking block behind even after its requested commit was rewritten away and pruned.
