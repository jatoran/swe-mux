...
### Stall diagnostics must not block or crash the daemon

`stall_watchdog.py` samples with `sys._current_frames()` on a Python thread, holding owned frame references rather than reading changing interpreter frames from a native watchdog.
The event loop publishes a timestamp only; it never creates or joins a diagnostic thread, acquires a diagnostic lock, or writes a trace.
Sampling requires the GIL, so a native GIL-held stall may have no stack sample.
Canary starvation records the coverage gap, and desktop-owned recovery remains independent of the daemon's GIL.
The sampler rotates its bounded trace even during a persistent stall; explanations retain the existing log and telemetry contracts.
Automatic `faulthandler.dump_traceback_later()` is forbidden; the ordinary fatal-crash handler remains enabled.
Recovery, process identity, and session-protection contracts: `../../../design/features/daemon-resilience.md`.
