...
# The GitHub organisation is undecided, so every repository URL on the site is
# built from this one constant and every link it produces carries `data-todo`.
# Filling it in is one edit here plus a regenerate.
# TODO(release): OWNER
REPO = "https://github.com/OWNER/swe-mux"
REPO_TODO = 'data-todo="TODO(release): OWNER"'
BLOB = f"{REPO}/blob/master"

# Every `data-todo` value the site is allowed to carry. `tools/check.mjs` fails
# on any other, so an unfilled URL cannot arrive quietly.
TODO_VALUES = ("TODO(release): OWNER", "blog URL", "repository URL")
...
def inline(md: str, base: str | None = None) -> str:
    ...
    def link(m: re.Match[str]) -> str:
        ...
        if base is not None and not re.match(r"^([a-z]+:|#|//)", href):
            ...
            return f'<a href="{BLOB}/{html.escape(path, quote=True)}" {REPO_TODO}>{text}</a>'
...
{nav}
    ...
    <div class="util">
      <a href="{REPO}" {REPO_TODO} aria-label="Source on GitHub" title="Source on GitHub">
        {GITHUB_MARK}
...
<footer>
  <div class="wrap">
    <div class="cols">
      ...
      <div>
        <h4>source</h4>
        <div><a href="{REPO}" {REPO_TODO}>github.com/OWNER/swe-mux</a></div>
        <div><code>.docs/design/00_OVERVIEW.md</code></div>
      </div>
      <div>
        <h4>license</h4>
        <div><a href="{BLOB}/LICENSE" {REPO_TODO}>Apache-2.0</a></div>
        <div><a href="{BLOB}/THIRD-PARTY-NOTICES.md" {REPO_TODO}>Third-party notices</a></div>
      </div>
    </div>
...
def build_changelog() -> str:
    ...
    while i < len(blocks):
        ...
        if b.kind == "h" and b.level == 2:
            ...
            href = links.get(name)
            shown = (
                f'<a href="{html.escape(href)}" {REPO_TODO}>{html.escape(name)}</a>'
                if href
...
def build_docs() -> str:
    ...
    for slug, title, paths in DOC_SECTIONS:
        ...
        for rel in paths:
            ...
            out.append(
                f'          <div class="t"><a href="{BLOB}/{repo_rel}" {REPO_TODO}>'
                f"{title_html}</a>"
...
def build_acknowledgements() -> str:
    ...
    out.append(
        ...
        f'<a href="{BLOB}/THIRD-PARTY-NOTICES.md" {REPO_TODO}>'
