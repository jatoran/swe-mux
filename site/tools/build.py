...
<!-- Generated by site/tools/build.py. Edit that source, not this file. -->
<html lang="en">
<head>
<meta charset="utf-8" />
...
def _agent_block(up: str) -> list[str]:
    ...
    return [
        ...
        f"<code>{html.escape(AGENT_GUIDE_URL, quote=False)}</code></a>, plain Markdown "
        "rather than a page, so an agent gets the text and not a layout. It tells your "
        "agent to ask before it installs anything.</p>",
        "      </div>",
    ]
...
def build_docs(up: str) -> str:
    ...
    body.append(
        '      <p class="note">For changing swe-mux rather than using it, '
        '<a href="contributing/">Developing swe-mux</a> is the maintainer page, and the only '
        "one here that sends you into the repository.</p>"
    )
...
def build_acknowledgements(up: str) -> str:
    ...
    out.append(
        '    <div class="lede"><p>Almost none of what swe-mux runs on was written here: the '
        "coding agents, the terminal emulator, the mesh network. "
...
{rows}
      ...
      reordered, nothing left off it. Below {MIN_VOTES} votes a request is not drawn here
      at all.</p>
    </div>"""
...
def build_posts_block() -> str:
    ...
    if not posts:
      ...
      <p class="prose">There is no first post yet. When there is, it appears here with
      its own permalink.</p>
    </div>"""
    out: list[str] = []
    if len(posts) > 1:
