...
| `/roadmap/` | `roadmap/index.html` | `content/roadmap.html`, hand-authored |
...
3. **Themes, and landed versus planned.**
   That is the honest resolution, and it is checkable against the changelog rather than against a schedule.

The page keeps a **deliberately not on the roadmap** section, because the boundaries (never actuates, never kills a process on suspicion, no cloud service) are the argument rather than a list of gaps.
