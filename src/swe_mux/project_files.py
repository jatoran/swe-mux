...
def project_land_verify_grant(root: str | Path) -> str:
    ...
    than to the default: corruption must never widen an authority, and this one ends in
    the daemon executing a script.

    The Project layer only; see `_authority_grant` on what the daemon actually enforces.
    """
    return _authority_grant(root, "land_verify_grant", SESSION_CONTROL_GRANTS, "draft")
