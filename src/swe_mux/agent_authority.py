...
AUTHORITY_FIELDS: dict[str, AuthorityField] = {
    ...
    for field in (
        ...
        AuthorityField(
            field="land_verify_grant",
            levels=("draft", "granted"),
            # Starts wide, unlike `land_grant` beside it, and the pair is the
            # clearest illustration of why these are two fields. That one says
            # who may *start* a land; this says what the daemon may *execute*
            # while running one. An agent working in a worktree already runs
            # arbitrary commands there, so approving its gate edits bought
            # little and stalled every parallel wave that touched the script -
            # while the thing approval genuinely protects, a gate somebody
            # *else* put on the branch, is refused by provenance rather than by
            # this level (`verify_provenance.py`).
            builtin="granted",
...
#: The fields that decide whether an agent still needs a human. Kept apart from
#: the envelope field because a surface that groups them wants to say "these
#: permit, that one discloses", and because `spends`-style validation of the
#: actuation set should not have to special-case a level that costs nothing and
#: permits nothing.
