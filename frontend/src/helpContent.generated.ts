    ...
    "sections": [
      ...
      },
      {
        ...
        "blocks": [
          ...
          {
            "kind": "p",
            "text": "That boundary is right as a cost decision and, repeated per conversation, is pure friction for a Project that has already decided it wants a timeline. scan_timeline_auto_enable in the Project config answers \"yes, always\" once. It only ever creates a grant for a run that has no row at all: a run the human switched off, or one an ended session disabled, stays off, because an off switch that re-arms itself is not an off switch. The snapshot reports auto_enable but never applies it - a read that started scanning would make opening the drawer a spending decision - so a brand-new run reads as off until its first trigger arms it. Turning the Project's permission off clears the flag, so re-permitting later does not silently re-arm every conversation."
          }
        ]
      }
    ]
    ...
    "sections": [
      {
        ...
        "blocks": [
          ...
          {
            "kind": "p",
            "text": "The worktree launcher is an explicit Git operation rather than a Project Action. It creates a named branch below the configured global worktree root through POST /api/git/worktrees, closes the launcher once that durable operation succeeds, then bootstraps and starts the selected backend through POST /api/git/worktrees/session. The browser immediately creates and focuses a client-only unpanned pending session at the worktree path. Its full-workspace setup surface leaves the durable pane tree unchanged, so selecting another session removes setup from view without disturbing existing splits. The pending row is replaced in place by the daemon session when setup and spawn finish. Moving elsewhere during setup is respected: completion updates the pending location without reclaiming focus. Its suggested checkout path is grouped by Project and branch below worktree_root, which defaults to <data_dir>/worktrees and is editable in Settings under Git. The resulting absolute path remains editable before creation, and changing the setting does not move existing worktrees. Whitespace entered in the branch field becomes -, keeping the Git branch and suggested filesystem path aligned."
          }
        ]
      }
    ]
