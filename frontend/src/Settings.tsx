...
type Config = {
  revision:number; host:string; port:number; data_dir:string; requires_auth:boolean
...
type UsageStatus = {
  enabled:boolean; refreshing:boolean
...
export function Settings({ onClose, cwd, initialSection='General' }: { onClose: () => void; cwd?:string; initialSection?:string }) {
  ...
  const refreshUsage = async () => {
    ...
    catch (error) { setErrors(current=>({...current,ccusage:(error as Error).message})) }
  }
  ...
  return <div class="settings-layer"><section class="settings-panel" ref={panel} role="dialog" aria-modal="true" aria-label="Settings">
    <header><div><span>CONFIG::V2</span><h2>Settings</h2></div><button onClick={() => { if(config) applyTheme(config.theme); onClose() }}>×</button></header>
    <main>
      ...
      <section><h3>Remote and security</h3><dl><dt>Bind</dt><dd>{draft.host}:{draft.port}</dd><dt>Authentication</dt><dd>{draft.requires_auth?'required':'loopback only / disabled'}</dd></dl><p>Bearer secrets are deliberately excluded from this surface and exported settings.</p></section>
