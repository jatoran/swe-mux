/**
 * The prompt an operator hands an agent to set up worktree verification **here**.
 *
 * It is drawn on the repository that needs it, which is what the strip's own copy now
 * says: the earlier "setting this up in another repository" framing named the wrong
 * repository, and the prompt's first line has always said "this repository".
 *
 * Auto-merge's whole value is that a machine runs the gate, and a repository with no
...
export function verifySetupPrompt(scriptName: string): string {
  ...
  return `Set up worktree verification for this repository, so swe-mux can auto-merge branches here.

## What the command is for

swe-mux's auto-merge queue merges a finished worktree branch onto the trunk with no human driving it.
...
The gate's process **exit code is the only verdict**. Zero merges the branch. Nonzero hands the request back to the agent that asked for it, with the tail of the output. Nothing reads the output to decide anything, and no model is involved at any point.
...
- **It must be bounded.** It runs on every merge. No watch mode, no interactive prompt, nothing that waits for input, nothing that runs until interrupted.
...
2. Deliberately break one test, run the command again, and confirm it exits **nonzero**. A gate that cannot fail is worse than no gate, because it merges everything.
...
Approval is a separate human act against the exact bytes, made in swe-mux under Git → Map → AUTO-MERGE → Verification settings → Review bytes → "Approve these bytes".
