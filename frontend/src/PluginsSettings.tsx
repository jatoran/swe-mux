  ...
  return <div class="plugins-settings">
    ...
    <section><h3>Installed plugins</h3>
      ...
      {catalogue.plugins.map(plugin=><article class={`plugin-card ${plugin.enabled?'enabled':''}`} key={plugin.id}>
        ...
        {plugin.enabled&&!!plugin.manifest?.panes.length&&<div class="plugin-contributions"><h4>Panes</h4>{plugin.manifest.panes.map(pane=><button disabled={!!busy||!projectId} title={pane.description||pane.command.command.join(' ')} onClick={()=>void run(pane.title,()=>mutate('POST',`/api/plugins/${plugin.id}/panes/${pane.id}`,projectContext))}>{pane.title}</button>)}</div>}
