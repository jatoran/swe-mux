      ...
      {overview?.worktrees.map(tree=>{
        ...
        return <article class="git-map-row" key={tree.path}>
          <button class="git-map-summary" aria-expanded={expanded} onClick={()=>setExpandedTree(expanded?'':tree.path)}>
            <span class={`git-map-rail ${tree.main?'main':''}`}>{tree.main?'●':'○'}</span><span class="git-map-identity"><strong>{tree.branch||`detached @ ${shortSha(tree.head)}`}</strong><small>{pathTail(tree.path)}{tree.main?' · main tree':''}</small></span>
            <span class="git-map-metrics">{total===0&&<em class="clean">clean</em>}{total>0&&<em class="local">{total} local</em>}{tree.comparisonCounts&&<><em>{tree.comparisonCounts.ahead} ahead</em><em>{tree.comparisonCounts.behind} behind</em></>}{upstream&&<em class="diverged">upstream {upstream.ahead?`↑${upstream.ahead}`:''}{upstream.behind?` ↓${upstream.behind}`:''}</em>}{attached.length>0&&<em class="live">{attached.length} live</em>}{tree.locked!==null&&<em class="warn">locked</em>}{tree.prunable!==null&&<em class="warn">prunable</em>}</span><span>{expanded?'−':'+'}</span>
          </button>
