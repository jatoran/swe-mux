...
type Props={onOpenNote:(scope:ProjectScope)=>void;onOpenSettings:(scope:ProjectScope)=>void;onClose:()=>void}

export function ProjectRegistry({onOpenNote,onOpenSettings,onClose}:Props){
  const [items,setItems]=useState<ProjectScope[]>([])
  const [selected,setSelected]=useState<ProjectScope|null>(null)
  const [showHidden,setShowHidden]=useState(false)
  const [error,setError]=useState('')
  const load=async()=>{try{const result=await api<{items:ProjectScope[]}>('GET',`/api/projects${showHidden?'?include_hidden=1':''}`);setItems(result.items);setError('')}catch(cause){setError(cause instanceof Error?cause.message:String(cause))}}
  ...
  return <div class="projects-layer" role="dialog" aria-modal="true" aria-label="Projects" onMouseDown={event=>event.target===event.currentTarget&&onClose()}>
    <section class="projects-panel"><header><div><span>PROJECT SHELF</span><h2>Projects</h2></div><label><input type="checkbox" checked={showHidden} onChange={event=>setShowHidden(event.currentTarget.checked)}/>hidden</label><button disabled={!selected?.root_exists} title={selected?'Open this project configuration in Settings':'Select a project first'} onClick={()=>selected&&onOpenSettings(selected)}>project settings</button><button onClick={onClose}>×</button></header>
      ...
      <div class="projects-body"><aside>
        ...
        </>:<div class="projects-placeholder"><strong>Projects own project configuration, project notes, and agent-run notes.</strong><p>Use the Notes shelf for one searchable view across project, agent-run, and app-owned space notes.</p></div>}</main></div>
    </section>
  </div>
}
