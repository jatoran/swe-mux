...
export function GuidedTutorial({hasProject,onNavigate,onExit,onComplete}:Props){
  ...
  const steps=useMemo<Step[]>(()=>[
    ...
    {id:'new-tab',eyebrow:'ADD A TAB',title:'Create a second terminal tab.',selectors:['[data-tutorial="new-tab"]'],action:{kind:'event',gate:{action:'session-launched',backend:'shell'},hint:'Click + and wait for the terminal to start'},targetLabel:'CLICK +',body:<><p>Click the highlighted <strong>+</strong>. This creates another shell in the focused pane so you can practice moving it.</p><p>The new tab must finish starting before the tour continues.</p></>},
    ...
    {id:'resources',eyebrow:'PROJECT RESOURCES',title:'Open the Project note.',selectors:['[data-tutorial="project-note"]'],action:{kind:'click',selectors:['[data-tutorial="project-note"]'],hint:'Click Project note'},targetLabel:'OPEN PROJECT NOTE',body:<><p>Click the Project note in the sidebar. Notes and Files become ordinary workspace tabs, so they can be stacked beside an agent or moved into their own pane.</p></>},
