...
export function GuidedTutorial({hasProject,onNavigate,onExit,onComplete}:Props){
  ...
  const steps=useMemo<Step[]>(()=>[
    ...
    // Anchored on the pane header's note chip rather than a sidebar row: the chip is the
    // one note affordance rendered on both desktop and mobile, and by this step the tour
    // has launched two shell sessions, so it is guaranteed present. A click-gated step has
    // no Next button, so an anchor that can be absent strands the tour with only Exit.
    {id:'resources',eyebrow:'PROJECT RESOURCES',title:'Open a session note.',selectors:['[data-tutorial="session-note"]'],action:{kind:'click',selectors:['[data-tutorial="session-note"]'],hint:'Click the note chip'},targetLabel:'OPEN SESSION NOTE',body:<><p>Click <strong>note</strong> in this terminal’s header. Notes become ordinary workspace tabs, so they can be stacked beside an agent or moved into their own pane.</p><p>The Project note and the file browser live in the right-hand panel, alongside a Project’s other notes.</p></>},
    {id:'features',eyebrow:'FIND ANYTHING',title:'Open the main menu.',selectors:['[data-tutorial="menu"]'],action:{kind:'click',selectors:['[data-tutorial="menu"]'],hint:'Click menu'},targetLabel:'CLICK MENU',body:<><p>The main menu is the map to History, session notes, processes, prompt templates, usage, notifications, automation, and Settings.</p><p>Open it now to see those real destinations.</p></>},
    {id:'feature-menu',eyebrow:'MAIN FEATURES',title:'Everything else stays one click away.',selectors:['[data-tutorial="main-menu"]'],body:<><p>Browse History and notes, inspect all processes, reuse prompt templates, monitor usage and notifications, configure automation, or open Settings from this menu.</p><p>Press <kbd>Ctrl Alt P</kbd> for the command palette. <kbd>Ctrl Alt T</kbd> opens a terminal; <kbd>Ctrl Alt ←/→</kbd> moves focus between panes.</p></>},
