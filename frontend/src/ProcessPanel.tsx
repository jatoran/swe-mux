...
export type FleetSnapshot = {
  ...
  daemon?:{pid:number;processes:number;cpu_pct:number;memory_bytes:number}
...
type Props={
  initialSessionId?:string|null; sessions:Session[];projects:Project[];onClose:()=>void
...
export function ProcessPanel({initialSessionId=null,sessions,projects,onClose,onAttached}:Props) {
  ...
  return <div class="process-layer" onPointerDown={event=>{if(event.target===event.currentTarget)onClose()}}><section ref={panel} class="process-panel unified" role="dialog" aria-modal="true" aria-label={selectedSession?`Processes for ${selectedSession.name}`:'All processes'}>
    ...
    <div class="process-fleet-list">{error&&<p class="process-error" aria-live="assertive">{error}</p>}{!snapshot&&!error&&<p class="process-empty">Loading process trees…</p>}{snapshot&&!snapshot.available&&<p class="process-empty">{snapshot.diagnostic}</p>}{projectProcessGroups.map(group=><section class="process-project-group" key={group.id}><h2>project::{group.label}</h2>{group.groups.map(renderGroup)}</section>)}{snapshot?.available&&!sessionGroups.length&&<p class="process-empty">No matching live sessions.</p>}</div>
  </section></div>
}
