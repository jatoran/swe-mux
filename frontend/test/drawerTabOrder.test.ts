...
test('a stored arrangement round-trips and reports as custom', () => {
  const custom: DrawerTabId[] = ['files', 'notes', 'clipboard', 'commands', 'prompts', 'notifications']
...
test('a tab the stored order predates lands beside its default neighbour, not at the end', () => {
  ...
  assert.deepEqual(
    ...
    ['notifications', 'files', 'notes', 'clipboard', 'commands', 'prompts'],
  )
...
test('keyboard cycling walks the arranged order, not the registry order', () => {
  const custom: DrawerTabId[] = ['notes', 'files', 'clipboard', 'commands', 'prompts', 'notifications']
