...
test('terminals, previews, notes, file editors, and History share one pane',()=>{
  const note=noteResourceId('note','project-a'),secondNote=noteResourceId('note','release-plan'),scratchpad=noteResourceId('global-note','scratchpad'),file=noteResourceId('file','src/app.ts')
  ...
  layout=openTab(layout,secondNote,resourceLeaf('note',scratchpad))
  layout=openTab(layout,scratchpad,resourceLeaf('note',file))
  ...
  assert.deepEqual(parseNoteResourceId(scratchpad),{kind:'global-note',id:'scratchpad'})
