...
test('reading a reply parses its target and its verbatim/summary mode', () => {
...
test('session listing parses its filter and its scope independently', () => {
...
test('status and open queries survive run-together speech recognition output', () => {
...
test('help queries parse their category, and the help text never names the wake word', () => {
...
test('only navigation is safe to act on while the system is speaking', () => {
...
test('voice filters select the sessions their names promise', () => {
  const model = fleet()
...
test('a session list page speaks a page, says what is left, and resumes where it stopped', () => {
  const model = fleet()
...
test('a filtered list keeps each session\'s canonical number rather than renumbering', () => {
  const model = fleet()
...
test('a project list page numbers projects, honouring an explicit address', () => {
...
test('a help page is spoken as a numbered, closed list', () => {
...
test('help includes the commands the live fleet contributes', () => {
...
test('the uncategorized help page is a catalog in text and a pointer in speech', () => {
